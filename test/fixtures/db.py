...
from test.fixtures.ids import AYREON_MBID, ALT_J_MBID

__all__ = ['session', 'session_with_artist', 'folder_info', 'folder_info_with_unicode',
           'session_with_artist_including_unicode']
...
@pytest.fixture()
def folder_info_with_unicode():
    """
    Album info, track->items mapping pickle
    :return:
    """
    path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'an_awesome_wave.pkl')
    with open(path) as f:
        return pkl.load(f)
